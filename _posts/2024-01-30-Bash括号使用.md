...
There is a subtle syntactic difference with `( )`, though (see [bash reference](http://www.gnu.org/software/bash/manual/bashref.html#Command-Grouping)) ; essentially, a semicolon `;` after the last command within braces is a must, and the braces `{`, `}` **must** be surrounded by spaces.
